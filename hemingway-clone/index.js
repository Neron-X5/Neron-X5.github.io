...
Paste in something you're working on and edit away. Or, click the Write button and compose something new.`;
    ...
    function counters() {
        document.querySelector('#adverb').innerHTML = `You have used ${data.adverbs} adverb${
            data.adverbs > 1 ? 's' : ''
        }. Try to use ${Math.round(data.paragraphs / 3)} or less`;
        ...
        document.querySelector('#hardSentence').innerHTML = `${data.hardSentences} of ${data.sentences} sentence${
            data.sentences > 1 ? 's are' : ' is'
        } hard to read`;
        document.querySelector('#veryHardSentence').innerHTML = `${data.veryHardSentences} of ${
            data.sentences
        } sentence${data.sentences > 1 ? 's are' : ' is'} very hard to read`;
    }
    ...
    function checkPrewords(words, originalWords, match) {
        ...
        if (preWords.indexOf(words[index - 1]) >= 0) {
            ...
            let next = checkPrewords(words.slice(index + 1), originalWords.slice(index + 1), match);
